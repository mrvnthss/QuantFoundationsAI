...
\[
    \Pr{\abs{S_n - \Ex{S_n}} > t} \leq 2 \exp(-\frac{2t^2}{\sum_{i=1}^n (b_i - a_i)^2}).
\]
\end{theorem}
...
\[
    \Pr{S_n - \Ex{S_n} > t} \leq \exp(-\frac{2t^2}{\sum_{i=1}^n (b_i - a_i)^2})
\]
as the bound for the lower tail follows analogously. For any $s>0$, we have
\[
    \Pr{S_n - \Ex{S_n} > t} = \Pr{\exp(s(S_n - \Ex{S_n})) > \exp(st)} \leq \exp(-st) \Ex{\exp(s(S_n - \Ex{S_n}))},
\]
...
\[
    \Pr{\abs{\bar X - \ex{\bar X}} > t} = \Pr{\abs{S_n - \Ex{S_n}} > nt} \leq 2 \exp(-\frac{2n^2t^2}{\sum_{i=1}^n (b_i - a_i)^2}).
\]
...
\[
    \Pr{\abs{\bar X - \ex{X_1}} > t} \leq 2 \exp(-\frac{2nt^2}{(b - a)^2}).
\]
For random variables $X_i$ taking values in the unit interval, this reduces to
\[
    \Pr{\abs{\bar X - \ex{X_1}} > t} \leq 2 \e^{-2nt^2}.
\]
\end{remark}
